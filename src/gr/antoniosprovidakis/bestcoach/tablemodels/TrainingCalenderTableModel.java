 ...
 */
public class TrainingCalenderTableModel extends AbstractTableModel {
    ...
    private final String SELECTFROMTRAININGS = "SELECT id, date, duration FROM trainings ORDER BY date ASC";

    public TrainingCalenderTableModel(Connection conn) {
        try {
            connection = conn;
            ...
            while (resultSet.next()) {
                ...
                newRow.add(resultSet.getInt("duration"));
                data.add(newRow);
            }
    ...
    public void deleteRow(int row) {
        try {
            ...
            String deleteQuery = "delete from trainings where id=?";
    ...
    public void addRow(ArrayList newRow) {
        try {
            ...
            int duration = (Integer) newRow.get(1);

            String insertQuery = "insert into trainings(date, duration) values(?,?)";
            insertMatch = connection.prepareStatement(insertQuery);

            insertMatch.setDate(1, date);
            insertMatch.setInt(2, duration);
